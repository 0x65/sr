use enet::{
    Address, BandwidthLimit, ChannelLimit, Enet, Error, InitializationError,
};
...
impl Network {
    ...
    pub fn create_manager(&self, config: NetworkConfig) -> Result<NetworkManager, Error> {
        let host = self.enet.create_host::<()>(
            config.local_addr.map(|a| Address::new(a, FRONTEND_PORT)).as_ref(),
            NUM_CHANNELS,
            ChannelLimit::Maximum,
            BandwidthLimit::Unlimited,
            BandwidthLimit::Unlimited,
        )?;
        Ok(NetworkManager::new(host))
    }
}
